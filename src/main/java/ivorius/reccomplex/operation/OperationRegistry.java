...
public class OperationRegistry
{
    ...
    public static void writeBestPreview(ByteBuf buffer, Operation operation)
    {
        ...
        while ((nbt = dummyOperation(operation, level++)) != null)
        {
            if (canSend(nbt))
            {
                new RCPacketBuffer(buffer).writeNBTTagCompoundToBuffer(nbt);
                return;
            }
        }

        new RCPacketBuffer(buffer).writeNBTTagCompoundToBuffer(null);
    }

    protected static boolean canSend(NBTTagCompound danglingNBT)
    {
        ByteBuf temp = Unpooled.buffer();
        new RCPacketBuffer(temp).writeNBTTagCompoundToBuffer(danglingNBT);
