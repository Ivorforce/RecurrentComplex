...
import ivorius.structuregen.gui.*;
...
{
    public static ResourceLocation textureBackground = new ResourceLocation(StructureGen.MODID, StructureGen.filePathTextures + "guiEditInventoryGen.png");
    ...
    protected void actionPerformed(GuiButton button)
    {
        if (button.enabled)
        {
            ...
            {
                StructureGen.chEditInventoryGenerator.sendSaveEdit(mc.thePlayer, inventoryGenerator, key);
            ...
            else if (button.id == 2)
            {
                ...
                StructureGen.chGuiAction.sendActionToServer("igSelectCol", currentColShift);
            }
            else if (button.id == 3)
            {
                scrollTo(currentColShift - 1);
